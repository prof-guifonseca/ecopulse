...
export function ArenaPage() {
  ...
  const [battleMasteryAtStart, setBattleMasteryAtStart] = useState<ArenaRivalMastery | undefined>(
    undefined,
  );
  ...
  const handleBattleComplete = useCallback(
    (result: BattleResult) => {
      ...
      if (result.outcome === 'win') {
        ...
        showToast(
          `+${reward.total} XP de teste · ${opponent?.victoryLine ?? 'Derrota sem custo.'}`,
          'info',
          4200,
        );
      } else {
      ...
      <header className="pt-1">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h1 className="t-headline max-w-full leading-none break-words">Teste de Loadout</h1>
            <p className="t-caption mt-1">Vestiário · rounds simulados</p>
          </div>
          <span className="border-soft bg-tint-1 t-caption shrink-0 rounded-full px-3 py-1">
            0 tokens por treino
          </span>
        </div>
      </header>
      ...
      <section className="space-y-3">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span className="t-caption">
            {defeatedOpponents.length}/{orderedOpponents.length} superados
          </span>
        </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="t-caption mt-1">
            Nv {level} · {tokens} tokens
          </p>
          <BattleStatChips
            stats={playerFighter.stats}
            compact
            className="mt-3 max-w-[calc(100vw-152px)]"
          />
        </div>
      </div>
    </Card>
...
function ArenaSetup({
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-black/46 to-transparent"
      />
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="t-eyebrow">{stage.name}</p>
          <h2 className="t-headline mt-1 truncate">{opponent.name}</h2>
          <p className="t-body-sm mt-1 line-clamp-2">&ldquo;{opponent.quote}&rdquo;</p>
        </div>
      ...
      <div className="relative z-10 mt-4 grid min-h-[178px] grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-end gap-2">
        <div
          ...
          style={{
            background: `radial-gradient(ellipse, ${stage.palette.floor} 0%, transparent 68%)`,
          }}
        />
        <SetupFighter
          name={playerName}
          title={playerFighter.title}
          loadout={playerFighter.loadout}
        />
        <div className="t-title relative z-10 mb-16 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/32 text-[var(--accent-gold)]">
          VS
        </div>
        <SetupFighter
          name={opponent.name}
      ...
      <p className="t-caption relative z-10 mt-3 text-center">
        {unlocked
...
function SetupFighter({
  ...
  return (
    <div className="relative z-10 min-w-0 text-center">
      <div className="mx-auto flex h-32 w-[min(39vw,8rem)] items-end justify-center rounded-t-full rounded-b-[44%] bg-black/12">
        <Avatar loadout={loadout} size="xl" alt={name} mirror={mirror} pose="battleReady" />
      </div>
      <p className="t-title mt-2 truncate">{name}</p>
      <p className="t-caption truncate">{title}</p>
    </div>
  );
}
...
function lockHint(opponent: ArenaOpponent, orderedOpponents: ArenaOpponent[]) {
  const previous = [...orderedOpponents].reverse().find((item) => item.order < opponent.order);
  return previous
    ? `Supere ${previous.name} para liberar o treino com ${opponent.name}.`
    : 'Treino bloqueado na trilha.';
}
